...
"""
Path Planning Visualization with WASD Movement and Obstacle Creation
"""
...
#     pygame.display.set_caption("Path Planning - WASD Movement with Obstacles")
...
#     # Toggle for buffer zone visibility
#     show_buffer = True
...
#         # Rectangle button
#         rect_button = Button(
#             button_x, HEIGHT + 5, BUTTON_WIDTH, BUTTON_HEIGHT, 
...
#         dot_collides = False
#         for obstacle in obstacles:
#             # Always check collisions with buffer zone, even if not displayed
...
#         # Draw current mode and buffer status indicators
#         mode_text = f"Current Tool: {current_shape.name if current_shape != Shape.NONE else 'None'}"
...
def main():
    ...
    # Make window resizable
    screen = pygame.display.set_mode((WIDTH, HEIGHT + TOOLBAR_HEIGHT), pygame.RESIZABLE)
    pygame.display.set_caption("Path Planning - WASD Movement with Grid")
    ...
    while running:
        ...
        # Get cell info under mouse
        if mouse_pos[1] < HEIGHT:
            cell = grid.get_cell(mouse_pos[0], mouse_pos[1])
            if cell:
                row, col = cell
                cell_status = "Occupied" if grid.grid[row][col] == 1 else "Free"
                cell_text = f"Cell: [{col},{row}] - {cell_status}"
                cell_surf = font.render(cell_text, True, (200, 200, 200))
                cell_rect = cell_surf.get_rect(left=10, top=HEIGHT + 25)
