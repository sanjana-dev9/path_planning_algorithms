...
class PathFinder:
    """
    A class that implements A* algorithm for path planning.
    ...
    def start_planning(self):
        """Begin the path planning process using A* algorithm."""
    ...
    def heuristic(self, cell):
        ...
        # Using Manhattan distance for orthogonal movements
        # or Euclidean distance for allowing diagonal movements
        row, col = cell
    ...
    def astar_step(self):
        ...
        if not self.frontier:
            # Priority queue: (f_score, unique_identifier, (row, col))
            # f_score = g_score (cost so far) + heuristic (estimated cost to goal)
            # unique_identifier is used to break ties consistently
    ...
    def draw(self, screen):
        ...
        if hasattr(self, 'came_from') and hasattr(self, 'cost_so_far'):
            ...
            for cell in self.visited_cells:
                ...
                if cell in self.cost_so_far:
                    ...
                    # For A*, optionally show f-score (cost+heuristic) instead of just g-score
                    if self.show_decision_process:
                        f_score = cost + self.heuristic(cell)
                        cost_text = f"{cost:.1f}+{self.heuristic(cell):.1f}"
                    else:
                        cost_text = f"{cost:.1f}"
        ...
        for row, col in self.frontier:
            ...
            if hasattr(self, 'cost_so_far') and (row, col) in self.cost_so_far:
                ...
                if self.show_decision_process:
                    f_score = cost + self.heuristic((row, col))
                    cost_text = f"{cost:.1f}+{self.heuristic((row, col)):.1f}"
                else:
                    cost_text = f"{cost:.1f}"
